<?php
	class Posts extends db{
		...
		/*public function createPost($postId,$title,$detail){
			$sql = "INSERT INTO post(postId,title,detail) VALUES(?,?,?)";
			$result = $this->dbcon->prepare($sql);
			$result->execute(array($postId,$title,$detail));
		}*/
		public function createPost($title,$detail){
			$sql = "INSERT INTO post(title,detail) VALUES(?,?)";
			$result = $this->dbcon->prepare($sql);
			$result->execute(array($title,$detail));
		}
		
		public function updatePost($title,$detail,$postId){
